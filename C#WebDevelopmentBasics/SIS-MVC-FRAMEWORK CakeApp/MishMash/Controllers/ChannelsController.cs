...
using Type = MishMash.Models.Type;
...
namespace MishMash.Controllers
{
    public class ChannelsController : BaseController
    {
        ...
        {
            var channelViewModel = this.Db
                .Channels.Where(x => x.Id == id)
        ...
        public IHttpResponse Follow(int id)
        {
            ...
            if (!this.Db.UserChannels.Any(x => x.UserId == user.Id && x.ChannelId == id))
            {
        ...
        public IHttpResponse Unfollow(int id)
        {
            ...
            var userInChannel = this.Db.UserChannels.FirstOrDefault(
                x => x.UserId == user.Id && x.ChannelId == id);
            if (userInChannel != null)
            {
                this.Db.UserChannels.Remove(userInChannel);
                this.Db.SaveChanges();
            }
